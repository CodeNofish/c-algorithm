...
unsigned short do_crc_table(unsigned char *ptr, int len) {
    ...
    while (len--) {
        // 低位放高位
        // 原高位 和 数据 异或，根据表获取异或结果
        crc = (crc << 8) ^
              crc16tab[((crc >> 8) ^ *ptr++) & 0xff];
    }
    return crc;
}
...
//         << (0x0102 << 8);
    ...
    cout << setbase(16) << setiosflags(ios::showbase);
    cout << do_crc((unsigned char *) src, sizeof(src)) << endl;
    cout << do_crc_table((unsigned char *) src, sizeof(src)) << endl;
